  ...
  return new Promise(async(resolve, reject) => {
    try {
      ...
      if (status === 200) {
        ...
        if (typeof link === "undefined") return resolve({ creator: "@Rossy-Chan", mess: `Link Not Found` });
        resolve({url: link})
      } else {
        console.log('No result found')
      }
...
function TikTok(url){
  return new Promise(async(resolve, reject) => {
    try {
      ...
      if (data.mess) return resolve({ developer: '@Rossy-Chan', mess: data.mess });
      let ar = []
      let aud = []
      let wm = []
  ...
  return new Promise(async(resolve, reject) => {
    try {
      ...
      if (status === 200) {
        ...
        resolve({ developer: '@Rossy-Chan', mess: 'no result found' })
      }
    } catch (error) {
      console.error(error)
    }
