...
function MusiCally(url){
  return new Promise(async(resolve, reject) => {
    try {
      ...
      if (typeof vdlk === 'undefined') return resolve({ developer: '@Rossy-Chan', mess: 'Infalid link, no result found' })
  ...
  return new Promise(async(resolve, reject) => {
    try {
      ...
      if (status === 200) {
        ...
        if (typeof link === "undefined") return resolve({ creator: "@DarkYasiyaofc", mess: `Link Not Found` });
        resolve({url: link})
      } else {
        console.log('No result found')
      }
  ...
  return new Promise(async(resolve, reject) => {
    try {
      ...
      if (status === 200) {
        ...
        if (typeof link === 'undefined') return resolve({ developer: '@Rossy-Chan', mess: 'no result found' })
...
function TikTok(url){
  return new Promise(async(resolve, reject) => {
    try {
      ...
      if (data.mess) return resolve({ developer: '@DarkYasiyaofc', mess: data.mess });
      let ar = []
      let aud = []
      let wm = []
  ...
  return new Promise(async(resolve, reject) => {
    try {
      ...
      if (status === 200) {
        ...
        if (hasil.every(x => x === undefined)) return resolve({ developer: '@Rossy-Chan', mess: 'no result found' })
        resolve(respose)
      } else {
        resolve({ developer: '@DarkYasiyaofc', mess: 'no result found' })
      }
    } catch (error) {
      console.error(error)
    }
